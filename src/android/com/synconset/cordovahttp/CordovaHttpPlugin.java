...
public class CordovaHttpPlugin extends CordovaPlugin {
    ...
    private void enableSSLPinning(boolean enable) throws GeneralSecurityException, IOException {
        if (enable) {
            ...
            HttpRequest.addCert(context.getResources().getAssets().open(KEY_STORE_CLIENT_PATH),context.getResources().getAssets().open(KEY_STORE_TRUST_PATH),getHttpsKey());
    ...
    //  private static final String KEY_STORE_CLIENT_PATH = "pro.p12";//客户端要给服务器端认证的证书
    private static final String KEY_STORE_TRUST_PATH = "www/assets/client.bks";//客户端验证服务器端的证书库
    private String getHttpsKey() {
        return preferences.getString("HttpsSecrect", "");
    }
}
